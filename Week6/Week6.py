...
def member():
    if "name" in session and "username" in session and "password" in session:
        ...
        messages = ""  # 定义为空字符串
        for row in data:
            message_str = "{}:{}\n".format(row['name'], row['content'])  # 使用列名作为键
            messages += message_str  # 将每个消息字符串追加到 messages
        
        

        return render_template("Week6member.html", message= session["name"], messages= messages)
    else:
        return redirect("/")
