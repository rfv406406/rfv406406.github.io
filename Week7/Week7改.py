...
def member():
    if "name" in session and "username" in session and "password" in session:
        ...
        cursor.execute("SELECT member.name, message.id, message.content FROM member INNER JOIN message ON member.id=message.member_id")
        ...
        return render_template("Week7member 改.html", message = session["name"], messages = messages)
    else:
        return redirect("/")
